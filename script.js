...
const locations = {
  ...
  // 22:00 — home.
  // Plus code 4M35+6VG Jatihandap, Bandung
  dropoff: {
    name: "Drop Off",
    time: "22:00",
    label: "Home",
    ...
    lat: -6.896938,
    lng: 107.659703,
    googleMapsUrl: "https://www.google.com/maps/search/?api=1&query=-6.896938,107.659703"
  }
...
/* ---------------------------------------------------------
   ...
   --------------------------------------------------------- */
const mapConfig = {
  defaultCenter: [-6.8991, 107.6334],  // [latitude, longitude]
  defaultZoom: 13,
  minZoom: 11,
  maxZoom: 18,
  ...
  // Follow real roads instead of drawing straight lines.
  // Uses OSRM's free public server — no account, no API key.
  // Set to false if you prefer plain straight lines.
  useRoadRouting: true,
  routingProfile: "driving",   // "driving" | "bike" | "foot"
...
(function () {
  ...
  // Check in with the safety net in index.html — script.js is alive,
  // so the scroll-reveal styles can stay.
  if (window.__revealGuard) window.clearTimeout(window.__revealGuard);
  ...
  (function routeMap() {
    ...
    // Leaflet comes from a CDN. If it didn't arrive (offline, blocked network),
    // show a plain message instead of an empty grey box.
    if (typeof L === "undefined") {
      const wrap = host.parentElement;
      if (wrap) {
        wrap.innerHTML = '<p class="map-fallback">The map needs an internet ' +
                         'connection.<br>Everything else on this page works offline.</p>';
      }
      ["#btnViewRoute", "#btnCenterMap", "#btnLockMap"].forEach(function (sel) {
        const el = $(sel);
    ...
    let userMoved = false;   // stop auto-refitting once the map is touched

    // --- draws a line and, if routing is on, snaps it to real roads -----
    // A straight line is drawn immediately so the map is never empty, then
    // it is replaced by the road geometry as soon as OSRM answers. If the
    // request fails, the straight line simply stays.
    function drawRoute(points, style) {
      const line = L.polyline(points, style).addTo(map);
      if (!mapConfig.useRoadRouting || points.length < 2 || !window.fetch) return line;

      // OSRM wants lng,lat pairs separated by semicolons
      const coords = points.map(function (p) { return p[1] + "," + p[0]; }).join(";");
      const url = mapConfig.routingUrl + mapConfig.routingProfile + "/" +
      ...
      fetch(url)
        ...
        .then(function (data) {
          ...
          const snapped = route.geometry.coordinates.map(function (c) {
            return [c[1], c[0]];
          });
          line.setLatLngs(snapped);
          snapped.forEach(function (ll) { allPoints.push(ll); });
          if (!userMoved) fitRoute();
        })
    ...
    if (nightRide.enabled) {
      ...
      if (nightLatLngs.length > 1) {
        drawRoute(nightLatLngs, {
          color: mapConfig.nightRideColor,
          weight: 3.5,
          opacity: 0.85,
          dashArray: mapConfig.nightRideStyle === "dashed" ? "1 10" : null,
          lineCap: "round",
          lineJoin: "round"
        });
      }
